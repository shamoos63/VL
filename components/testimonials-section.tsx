...
const SpainFlag = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
    ...
    ></path>
    <path
      d="M8.36,16.058c-.153-.062-.39-.098-.653-.102v-.76c.094-.041,.034-.115-.013-.159,.02-.038,.092-.057,.056-.115h.043v-.261h-.912v.261h.039c-.037,.059,.039,.078,.057,.115-.047,.042-.108,.118-.014,.159v.873c-.644,.133-.611,.748,0,.945v.35c-.59,.154-.415,.567-.193,.896l.057,.053c.123-.103,.136-.31,.031-.429,.032-.018,.067-.036,.105-.053v.932c-.055,.018-.069,.093-.021,.126,.005,.064-.059,.089-.113,.081,0,.026,0,.236,0,.268-.045,.024-.113,.031-.170,.028v.401h0v.215c.459-.068,.895-.068,1.352,0v-.186s0,0,0,0v-.108s0,0,0,0v-.107s0,0,0,0v-.107s0,0,0,0v-.108c-.056,.002-.124-.004-.169-.028,0-.033,0-.241,0-.269-.055,.008-.119-.018-.113-.083,.045-.034,.03-.107-.024-.124v-1.29c.421-.192,.383-.772,0-.8v-.224c.575,.035,.796,.314,.653-.392Z"
      fill="#9b8028"
    ></path>
    <path
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          } else if (element === ctaRef.current) {
            setCtaVisible(true)
          } else {
            // Handle testimonial cards
            const index = cardRefs.current.indexOf(element as HTMLDivElement)
            if (index !== -1) {
  ...
  return (
    <section className="py-20 bg-gray-50" dir={isRTL ? "rtl" : "ltr"}>
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-vl-blue mb-6 font-heading">{t("testimonials.title")}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <Card
              ...
              className={`relative overflow-hidden hover:shadow-2xl hover:scale-105 transition-all duration-500 cursor-pointer group border-2 hover:border-vl-yellow/30 ${
            ...
            >
              <CardContent className="p-6">
                <div className="absolute top-4 right-4 text-vl-yellow opacity-20 group-hover:opacity-40 transition-opacity duration-300">
                  <Image src="/quotation_marks.svg" alt="Quote" width={32} height={32} />
                </div>
                ...
                <div className="flex items-center mb-4">
                  <div className="w-20 h-20 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center mr-4 text-4xl hover:scale-110 transition-all duration-300 shadow-lg group-hover:shadow-xl group-hover:from-vl-yellow/10 group-hover:to-vl-blue/10 border-2 border-transparent group-hover:border-vl-yellow/20">
                    <span className="hover:animate-pulse transition-all duration-300 group-hover:scale-110">
                      {testimonial.flag}
                    </span>
                  </div>
        ...
        >
          <div className="bg-vl-blue rounded-2xl p-8 max-w-4xl mx-auto hover:shadow-2xl transition-all duration-500 hover:scale-105">
            <h3 className="text-2xl font-bold text-white mb-4">{t("testimonials.join.title")}</h3>
            <p className="text-white/90 mb-6">{t("testimonials.join.subtitle")}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <div className="text-center hover:scale-110 transition-transform duration-300">
                <div className="text-3xl font-bold text-vl-yellow">AED 1.7B+</div>
                <div className="text-white/80">{t("testimonials.total.value")}</div>
              </div>
              <div className="text-center hover:scale-110 transition-transform duration-300">
                <div className="text-3xl font-bold text-vl-yellow">15+</div>
                <div className="text-white/80">{t("testimonials.years.experience")}</div>
              </div>
              <div className="text-center hover:scale-110 transition-transform duration-300">
                <div className="text-3xl font-bold text-vl-yellow">98%</div>
                <div className="text-white/80">{t("testimonials.client.satisfaction")}</div>
              </div>
            </div>
          </div>
        </div>
