const translations = {
  // Navigation
  "nav.home": "Главная",
  "nav.properties": "Недвижимость",
  "nav.areas": "Районы",
  "nav.evaluation": "Оценка недвижимости",
  "nav.blog": "Блог",
  "nav.about": "О нас",
  "nav.contact": "Контакты",
  "nav.search.placeholder": "Поиск недвижимости...",
  "nav.language": "Язык",

  // Hero Section
  "hero.title": "Найдите недвижимость своей мечты в",
  "hero.title.highlight": "Дубае",
  "hero.subtitle": "Роскошные дома, инвестиционные возможности и персональный сервис, которому можно доверять",
  "hero.search.button": "Поиск недвижимости",
  "hero.stats.properties": "Продано объектов",
  "hero.stats.value": "Общая стоимость",
  "hero.stats.experience": "Лет опыта",
  "hero.stats.satisfaction": "Удовлетворенность клиентов",

  // Property Search Form
  "search.property.type": "Тип недвижимости",
  "search.location": "Местоположение",
  "search.price.range": "Ценовой диапазон",
  "search.bedrooms": "Спальни",
  "search.apartment": "Квартира",
  "search.villa": "Вилла",
  "search.townhouse": "Таунхаус",
  "search.penthouse": "Пентхаус",
  "search.1.bedroom": "1 спальня",
  "search.2.bedrooms": "2 спальни",
  "search.3.bedrooms": "3 спальни",
  "search.4plus.bedrooms": "4+ спальни",

  // Locations
  "location.dubai.marina": "Дубай Марина",
  "location.downtown": "Центр Дубая",
  "location.palm.jumeirah": "Палм Джумейра",
  "location.jvc": "Джумейра Виллидж Серкл",
  "location.business.bay": "Бизнес Бей",
  "location.dubai.maritime": "Дубай Маритайм Сити",

  // Property Listings
  "properties.title": "Рекомендуемая недвижимость",
  "properties.subtitle":
    "Откройте для себя исключительную недвижимость, тщательно отобранную Викторией Ланкастер за их инвестиционный потенциал и роскошную привлекательность",
  "properties.filter.all": "Вся недвижимость",
  "properties.filter.featured": "Рекомендуемые",
  "properties.filter.ready": "Готовые",
  "properties.filter.offplan": "На стадии строительства",
  "properties.view.details": "Подробности",
  "properties.schedule.tour": "Запланировать осмотр",
  "properties.view.all": "Посмотреть всю недвижимость",
  "properties.status.ready": "Готово",
  "properties.status.offplan": "На стадии строительства",
  "properties.status.featured": "Рекомендуемое",

  // Properties Detail Page
  "properties.back.to.listings": "Назад к объектам",
  "properties.watch.video": "Смотреть видео",
  "properties.tab.overview": "Обзор",
  "properties.tab.features": "Особенности",
  "properties.tab.amenities": "Удобства",
  "properties.overview": "Обзор недвижимости",
  "properties.bedrooms": "Спальни",
  "properties.bathrooms": "Ванные комнаты",
  "properties.area": "Площадь",
  "properties.location": "Местоположение",
  "properties.features": "Особенности недвижимости",
  "properties.amenities": "Удобства здания",
  "properties.contact.agent": "Связаться с агентом",
  "properties.contact.now": "Связаться сей��а����",
  "properties.schedule.viewing": "Запланировать просмотр",
  "properties.available.for.viewing": "Доступно для просмотра",
  "properties.similar": "Похожие объекты",

  // About Section
  "about.title": "О Виктории Ланкастер",
  "about.subtitle": "Ваш надежный партнер в сфере недвижимости Дубая с 2020 года",
  "about.quote": '"Я не просто продаю недвижимость, я разрабатываю стратегии богатства."',
  "about.description.1":
    "Имея более 15 лет опыта на рынках ОАЭ и Великобритании, Виктория Ланкастер - это имя, синонимичное доверию, результатам и долгосрочному видению. Как директор по инвестициям в Select Property, она успешно закрыла более 585 сделок с недвижимостью на сумму свыше 1,7 миллиарда дирхамов ОАЭ.",
  "about.description.2":
    "Ее портфолио охватывает премиальную жилую недвижимость, инвестиции на стадии строительства и инвестиции для сдачи в аренду, все тщательно подобранные для создания безопасных, высокодоходных портфелей для глобальных клиентов.",
  "about.trait.professional": "Профессионал",
  "about.trait.experienced": "Опытный",
  "about.trait.trustworthy": "Надежный",
  "about.trait.multilingual": "Многоязычный",
  "about.trait.best.deals": "Лучшие сделки",
  "about.trait.fast.service": "Быстрый сервис",
  "about.expertise.title": "Области экспертизы",
  "about.expertise.offplan": "• Инвестиции на стадии строительства (Дубай и Великобритания)",
  "about.expertise.luxury": "• Роскошная жилая недвижимость",
  "about.expertise.buytolet": "• Стратегия покупки для сдачи в аренду",
  "about.expertise.portfolio": "• Диверсификация портфеля",
  "about.expertise.forecasting": "• Прогнозирование рынка и аналитика",

  // Testimonials
  "testimonials.title": "Истории успеха клиентов",
  "testimonials.subtitle":
    "Послушайте инвесторов, которые доверили Виктории Ланкастер свое путешествие в недвижимость Дубая",
  "testimonials.join.title": "Присоединяйтесь к 585+ довольным инвесторам",
  "testimonials.join.subtitle":
    "Готовы начать свое инвестиционное путешествие в недвижимость Дубая? Позвольте Виктории помочь вам достичь ваших целей.",
  "testimonials.total.value": "Общая стоимость сделок",
  "testimonials.years.experience": "Лет опыта",
  "testimonials.client.satisfaction": "Удовлетворенность клиентов",

  // FAQ Section
  "faq.title": "Часто задаваемые вопросы",
  "faq.subtitle": "с Викторией Ланкастер",
  "faq.quote":
    '"Я выбрала эти вопросы, потому что они отражают то, что люди действительно спрашивают меня - не только о покупке в Дубае, но о том, как я думаю, как работаю и что ценю. Я верю, что ясность строит доверие. Когда вы знаете, чего ожидать, вы можете принимать лучшие решения."',
  "faq.quote.author": "— Виктория Ланкастер",

  // FAQ Questions and Answers
  "faq.q1": "Могут ли иностранцы покупать недвижимость в Дубае?",
  "faq.a1":
    "Абсолютно. Нерезиденты могут полностью владеть недвижимостью в специально отведенных зонах свободного владения по всему Дубаю. Это упрощенный процесс, который приветствует глобальных инвесторов без требования резидентства.",

  "faq.q2": "Я новичок в инвестициях в недвижимость - с чего начать?",
  "faq.a2":
    "Начните с ясности. Поймите свои инвестиционные цели - ищете ли вы долгосрочный доход от аренды, краткосрочную прибыль или будущий дом? Как только ваши приоритеты станут ясными, Виктория поможет вам построить стратегию вокруг них.",

  "faq.q3": "Какую доходность можно ожидать от инвестиций в недвижимость в Дубае?",
  "faq.a3":
    "Дубай предлагает одни из самых высоких доходностей от аренды в мире - обычно от 5% до 8,4%, а в некоторых случаях даже до 15%.",

  "faq.q4": "Нужно ли мне быть в Дубае, чтобы купить недвижимость?",
  "faq.a4": "Совсем нет. С доверенностью и цифровой подписью вы можете покупать удаленно из любой точки мира.",

  "faq.q5": "Могут ли международные покупатели получить ипотеку в Дубае?",
  "faq.a5":
    "Да. Отдельные банки предлагают варианты финансирования для нерезидентов, в зависимости от вашего финансового профиля и типа недвижимости.",

  "faq.q6": "Есть ли налоги при покупке недвижимости?",
  "faq.a6":
    "В Дубае нет подоходного налога на недвижимость или налога на прирост капитала. Основная плата - это 4% регистрационный сбор в Департаменте земли и недвижимости Дубая, плюс незначительные административные и юридические сборы.",

  "faq.q7": "Даст ли мне владение недвижимостью визу на жительство?",
  "faq.a7":
    "Да. Если вы инвестируете 750,000 дирхамов ОАЭ или более в жилую недвижимость, вы имеете право подать заявление на получение 2-летней возобновляемой визы на жительство через Департамент земли и недвижимости Дубая.",

  "faq.q8": "Что лучше: готовая недвижимость или на стадии строительства?",
  "faq.a8":
    "Это зависит от ваших целей и того, что доступно на рынке. Готовая недвижимость может сразу начать приносить доход, в то время как проекты на стадии строительства часто предлагают более низкие точки входа, привлекательные планы платежей и потенциал роста капитала.",

  "faq.q9": "Какие районы в Дубае предлагают лучшие инвестиционные возможности?",
  "faq.a9":
    "Лучшие районы включают: Дубай Марина, Центр Дубая, Палм Джумейра, Джумейра Виллидж Серкл (JVC), Бизнес Бей и Дубай Маритайм Сити. Эти районы предлагают сочетание сильного спроса на аренду, премиальной привлекательности образа жизни и солидной долгосрочной доходности.",

  "faq.q10": "Что делает работу с Викторией отличной от других?",
  "faq.a10":
    "Виктория не просто продает - она разрабатывает стратегии. Имея более 15 лет опыта, глубокие знания рынка и прочные связи с ведущими застройщиками, она обеспечивает высокоценные сделки для своих клиентов.",

  "faq.q11": "Как Виктория выбирает недвижимость для рекомендации?",
  "faq.a11":
    "Если она сама не стала бы в это инвестировать, она не будет это рекомендовать. Виктория лично проверяет каждую возможность на основе надежности застройщика, прогнозов ROI, долгосрочной стоимости и интуиции.",

  "faq.q12": "Каково это - работать с Викторией один на один?",
  "faq.a12":
    "Ожидайте прозрачности, спокойной уверенности и полной честности. Виктория глубоко слушает, стратегически думает и говорит вам то, что вам нужно услышать - не только то, что вы хотите услышать.",

  "faq.q13": "Почему Виктория работает только с избранными клиентами и застройщиками?",
  "faq.a13":
    "Потому что качество важнее количества и приводит к лучшим результатам. Сосредотачиваясь только на надежных застройщиках и серьезных покупателях, Виктория сохраняет свое время, энергию и фокус для сделок, которые имеют значение.",

  // Property Evaluation
  "evaluation.title": "Инструмент оценки недвижимости",
  "evaluation.subtitle": "Получите профессиональную оценку недвижимости от экспертной команды Виктории",
  "evaluation.hero.title": "Инструмент оценки недвижимости",
  "evaluation.hero.subtitle":
    "Получите мгновенную оценку стоимости вашей недвижимости с помощью экспертной системы оценки Виктории",
  "evaluation.hero.properties.evaluated": "Оценено объектов",
  "evaluation.hero.total.value": "Общая оцененная стоимость",
  "evaluation.hero.years.experience": "Лет опыта",
  "evaluation.form.title": "Детали недвижимости",
  "evaluation.contact.info": "Контактная информация",
  "evaluation.name": "Полное имя",
  "evaluation.name.placeholder": "Ваше полное имя",
  "evaluation.email": "Адрес электронной почты",
  "evaluation.email.placeholder": "your.email@example.com",
  "evaluation.phone": "Номер телефона",
  "evaluation.phone.placeholder": "+971 XX XXX XXXX",
  "evaluation.property.type": "Тип недвижимости",
  "evaluation.select.type": "Выберите тип",
  "evaluation.location": "Местоположение",
  "evaluation.select.location": "Выберите местоположение",
  "evaluation.bedrooms": "Спальни",
  "evaluation.select.beds": "Спальни",
  "evaluation.bathrooms": "Ванные комнаты",
  "evaluation.select.baths": "Ванные",
  "evaluation.area": "Площадь (кв. футы)",
  "evaluation.area.placeholder": "например, 1200",
  "evaluation.condition": "Состояние недвижимости",
  "evaluation.select.condition": "Выберите состояние",
  "evaluation.condition.excellent": "Отличное",
  "evaluation.condition.good": "Хорошее",
  "evaluation.condition.fair": "Удовлетворительное",
  "evaluation.condition.renovation": "Требует ремонта",
  "evaluation.year.built": "Год постройки",
  "evaluation.year.placeholder": "например, 2020",
  "evaluation.amenities": "Удобства",
  "evaluation.amenities.placeholder": "например, Бассейн, Спортзал, Парковка, Балкон",
  "evaluation.description": "Дополнительные детали",
  "evaluation.description.placeholder": "Любая дополнительная информация о недвижимости...",
  "evaluation.button": "Отправить на профессиональную оценку",
  "evaluation.success.title": "Спасибо за вашу заявку!",
  "evaluation.success.message":
    "Мы получили детали вашей недвижимости и свяжемся с вами как можно скорее с комплексной оценкой и анализом рынка.",
  "evaluation.success.contact.title": "Мы свяжемся с вами через:",
  "evaluation.success.new.evaluation": "Отправить другую оценку",

  // Contact Page
  "contact.title": "Свяжитесь с нами",
  "contact.subtitle": "Готовы начать свое путешествие в недвижимость Дубая? Давайте обсудим ваши инвестиционные цели.",
  "contact.form.title": "Отправить сообщение",
  "contact.first.name": "Имя",
  "contact.first.name.placeholder": "Ваше имя",
  "contact.last.name": "Фамилия",
  "contact.last.name.placeholder": "Ваша фамилия",
  "contact.email": "Адрес электронной почты",
  "contact.email.placeholder": "your.email@example.com",
  "contact.phone": "Номер телефона",
  "contact.phone.placeholder": "+971 XX XXX XXXX",
  "contact.budget": "Инвестиционный бюджет",
  "contact.budget.placeholder": "например, $500K - $1M",
  "contact.message": "Сообщение",
  "contact.message.placeholder": "Расскажите нам о ваших требованиях к недвижимости и инвестиционных целях...",
  "contact.send": "Отправить сообщение",
  "contact.info.title": "Контактная информация",
  "contact.phone.label": "Телефон",
  "contact.email.label": "Электронная почта",
  "contact.office.label": "Офис",
  "contact.hours.label": "Рабочие часы",
  "contact.hours.value": "Вс - Чт: 9:00 - 18:00",
  "contact.location": "Наше местоположение",
  "contact.why.choose.title": "П��чем�� выбрать В��кторию Ланкастер?",
  "contact.why.choose.experience": "15+ лет опыта в сфере недвижимости Дубая",
  "contact.why.choose.transactions": "585+ успешных сделок с недвижимостью",
  "contact.why.choose.value": "1,7+ миллиарда дирхамов ОАЭ общей стоимости сделок",
  "contact.why.choose.multilingual": "Многоязычный сервис (английский, арабский, русский)",
  "contact.why.choose.strategies": "Персонализированные инвестиционные стратегии",

  // Areas Page
  "areas.title": "Лучшие районы Дубая",
  "areas.subtitle": "Исследуйте самые востребованные районы Дубая и инвестиционные возможности",

  // Blog Page
  "blog.title": "Аналитика недвижимости",
  "blog.subtitle": "Экспертные аналитические данные, анализ рынка и инвестиционные рекомендации от Виктории Ланкастер",

  // Properties Page
  "properties.page.title": "Наша недвижимость",
  "properties.page.subtitle": "Откройте для себя исключительную недвижимость в самых престижных районах Дубая",

  // Footer
  "footer.description":
    "Ваш надежный партнер в сфере недвижимости Дубая. Строим будущее через стратегические инвестиции в недвижимость и персональный сервис.",
  "footer.quick.links": "Быстрые ссылки",
  "footer.contact.info": "Контактная информация",
  "footer.newsletter": "Оставайтесь в курсе",
  "footer.newsletter.description":
    "Получайте последние новости о недвижимости и обновления рынка прямо на вашу электронную почту.",
  "footer.email.placeholder": "Ваш адрес электронной почты",
  "footer.subscribe": "Подписаться",
  "footer.copyright": "© 2025 VL Real Estate. Все права защищены.",
  "footer.privacy": "Политика конфиденциальности",
  "footer.terms": "Условия обслуживания",
  "footer.sitemap": "Карта сайта",

  // Map related translations
  "map.reset.view": "Сбросить вид",
  "map.available.properties": "Доступные объекты",
  "map.selected.property": "Выбранный объект",
  "map.properties.found": "объектов найдено",
  "map.loading": "Загрузка карты...",

  // Properties view translations
  "properties.view.grid": "Сетка",
  "properties.view.map": "Карта",

  // Filter related translations
  "filters.title": "Фильтры",
  "filters.price.range": "Ценовой диапазон",
  "filters.property.type": "Тип недвижимости",
  "filters.property.types.villa": "Вилла",
  "filters.property.types.apartment": "Квартира",
  "filters.property.types.penthouse": "Пентхаус",
  "filters.property.types.mansion": "Особняк",
  "filters.status": "Статус",
  "filters.status.ready": "Готово",
  "filters.status.offplan": "На стадии строительства",
  "filters.bedrooms": "Спальни",
  "filters.bathrooms": "Ванные комнаты",
  "filters.any": "Любой",
  "filters.reset": "Сбросить",
  "filters.apply": "Применить",
  "filters.search": "Поиск",
  "filters.search.placeholder": "Поиск по названию или местоположению",
  "properties.no.results": "Недвижимость не найдена",
  "properties.try.different.filters": "Попробуйте другие фильтры или сбросьте, чтобы увидеть все объекты",

  // Common
  "common.loading": "Загрузка...",
  "common.error": "Ошибка",
  "common.success": "Успех",
  "common.close": "Закрыть",
  "common.save": "Сохранить",
  "common.cancel": "Отмена",
  "common.submit": "Отправить",
  "common.search": "Поиск",
  "common.filter": "Фильтр",
  "common.sort": "Сортировка",
  "common.view.more": "Посмотреть больше",
  "common.read.more": "Читать далее",
  "common.learn.more": "Узнать больше",
}

export default translations
